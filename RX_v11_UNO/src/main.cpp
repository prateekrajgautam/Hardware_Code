...
//#include<printf.h> not available for pio
#include <Wire.h>
...
void calculateDirectionAndBW(unsigned int *Address, int *BWAddress, int *meanAddress)
{
  ...
  *BWAddress = Adiff;
  ...
  //*mean= *mean%360;
  int printDetails = digitalRead(PrintDetailsPin);
  if (printDetails == LOW)
  {
...
void setup()
{
  ...
  //printf_begin();  depends on printf lib
  Serial.begin(9600);
  radio.begin();
  ...
  if (radio.available()) //Looking for the data.
  {
    ...
    */
    ...
    if (FLAG == 255)
    {
      ...
      for (int i = 0; i < 14; i++)
      {
        Serial.print(phiPresent[i]);
      }
      Serial.print("\nthetaPresent\t");
      for (int i = 0; i < 14; i++)
      {
        Serial.print(thetaPresent[i]);
      }
      Serial.print("\n");
