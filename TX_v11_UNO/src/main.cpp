...
void loop()
{
  ...
  //read from MPU+DMP THETA 
   // if programming failed, don't try to do anything
    ...
    // read THETA
    mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);THETA=(ypr[1] * r2d);
    //read from QMC PHI 
    PHI = readQMC(0, p2);
    //convert
...
//CHECK AND READ RADIO
  ...
  delay(200);
    ...
    // read THETA
    mpu.dmpGetYawPitchRoll(ypr, &q, &gravity);THETA=(ypr[1] * r2d); THETA = mod(THETA,360.0);
    
    // read PHI
    PHI = readQMC(0, p2); PHI = mod((PHI+360-90.0),360.0);//90* shift, Y points to DA
    
    int phi0=PHI, theta0=THETA;
